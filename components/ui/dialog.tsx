...
<<<<<<< HEAD
      ...
      >
        <motion.div
          initial={
            shouldReduceMotion ? false : { opacity: 0, y: 8, scale: 0.985 }
          }
        ...
        >
          {describedBy === fallbackDescriptionId && (
            <DialogPrimitive.Description
              ...
              className="sr-only"
            >
              Janela de diálogo com informações e ações relacionadas ao item
              selecionado.
            </DialogPrimitive.Description>
          )}
          {children}
...
=======
          'ige-scrollbar bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid max-h-[calc(100dvh-2rem)] w-[95vw] max-w-4xl translate-x-[-50%] translate-y-[-50%] gap-4 overflow-y-auto rounded-lg border p-4 shadow-lg duration-200 sm:p-6',
          className,
        )}
      >
        {describedBy === fallbackDescriptionId && (
...
>>>>>>> cf9bfd381ed01be019ac1b6239f1b3f515cbf074
        {showCloseButton && (
          <DialogPrimitive.Close
            ...
            className="ring-offset-background focus:ring-ring data-[state=open]:bg-accent data-[state=open]:text-muted-foreground absolute top-4 right-4 rounded-xs opacity-70 transition-all hover:scale-105 hover:opacity-100 active:scale-95 focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4"
          >
            <XIcon />
...
=======
      className={cn('flex min-w-0 flex-col gap-2 pr-6 text-center sm:text-left', className)}
>>>>>>> cf9bfd381ed01be019ac1b6239f1b3f515cbf074
      {...props}
    />
  );
