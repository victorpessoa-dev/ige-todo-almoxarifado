...
import { AlertCircle, AlertTriangle, CalendarDays, Check, CircleSlash, Clipboard, ClipboardCheck, Clock3, ListPlus, PackagePlus, Play, Plus, Save, ShoppingCart, Trash2, X } from 'lucide-react'
...
import { getCompraQuantidade, getInventoryCategory, makeReposicaoBlocks, makeReposicaoLine, normalizeCategory } from '@/lib/inventory/replenishment'
...
export default function RevisoesPage() {
  ...
  const activeSolicitantes = useMemo(() => solicitantesCompra.filter((item) => item.ativo !== false), [solicitantesCompra])
  const activeCentrosCusto = useMemo(() => centrosCusto.filter((item) => item.ativo !== false), [centrosCusto])
  ...
  const paginatedRotinas = useMemo(() => paginateItems(rotinas, safeRotinasPage), [rotinas, safeRotinasPage])
  const paginatedRevisoes = useMemo(() => paginateItems(revisoes, safeRevisoesPage), [revisoes, safeRevisoesPage])
  const paginatedPendencias = useMemo(() => paginateItems(pendencias, safePendenciasPage), [pendencias, safePendenciasPage])
  const selectedPendenciaItems = useMemo(() => pendencias.filter((item) => selectedPendencias.includes(item.id)), [pendencias, selectedPendencias])
  const pendenciaGroups = useMemo(() => {
    ...
    return Array.from(groups.values())
      .map((group) => ({
        ...group,
        items: group.items.sort((a, b) => String(a.nome_snapshot || '').localeCompare(String(b.nome_snapshot || ''), 'pt-BR'))
      }))
      .sort((a, b) => a.category.localeCompare(b.category, 'pt-BR'))
  }, [paginatedPendencias])
  const selectedReposicaoBlocks = useMemo(() => makeReposicaoBlocks(
  ...
  const reposicaoListGroups = useMemo(() => {
    ...
    pendencias.forEach((item) => {
      ...
      const key = normalizeCategory(category) || 'sem-categoria'
      const line = makeReposicaoLine({ produto, quantidade: getPendenciaQuantidade(item) })
  ...
  const pasteListText = useMemo(() => {
    const groups = pasteCategory === 'todas'
      ? reposicaoListGroups
      : reposicaoListGroups.filter((group) => group.key === pasteCategory)

    return groups
      .map((group) => [group.category.toLocaleUpperCase('pt-BR'), ...group.lines].join('\n'))
      .join('\n\n')
  ...
  const getSolicitanteCentroCusto = (solicitante) => {
    ...
    return activeCentrosCusto.find((item) => item.id === solicitante.centro_custo_id) || solicitante.centros_custo || null
  }
  ...
  const reload = async () => {
    ...
    try {
      ...
      toast.error(getUserMessage(error, 'NÃ£o foi possÃ­vel carregar as revisÃµes.'))
  ...
  const submitRoutine = async (event) => {
    ...
    } catch (error) { toast.error(getUserMessage(error, 'NÃ£o foi possÃ­vel salvar a rotina.')) }
  }
  ...
  const removeRoutine = async () => {
    ...
    } catch (error) { toast.error(getUserMessage(error, 'NÃ£o foi possÃ­vel excluir a rotina.')) }
  }
  const treatItem = async (item, status) => {
    try {
      await updateReviewItem(item.id, status, item.observacao, item.quantidade_contada)
      const updatedItem = { ...item, status, revisado_em: new Date().toISOString(), quantidade_contada: item.quantidade_contada || null }
      const updateReview = (review) => review.id !== item.revisao_id ? review : { ...review, status: 'em_andamento', revisoes_estoque_itens: review.revisoes_estoque_itens.map((current) => current.id === item.id ? updatedItem : current) }
      ...
      if (['repor', 'em_falta'].includes(status)) {
        setPendencias((current) => current.some((pending) => pending.id === item.id) ? current.map((pending) => pending.id === item.id ? updatedItem : pending) : [{ ...updatedItem, revisoes_estoque: { id: selectedReview?.id, status: selectedReview?.status, rotinas_revisao: selectedReview?.rotinas_revisao } }, ...current])
      } else {
        setPendencias((current) => current.filter((pending) => pending.id !== item.id))
      }
    } catch (error) { toast.error(getUserMessage(error, 'NÃƒÂ£o foi possÃƒÂ­vel registrar o item.')) }
  }
  const moveReviewNextDay = async (review) => {
    try {
      ...
      const updatedReview = { ...review, status: 'pendente', agendada_para: next, notificar_em: next, adiada_ate: next, adiada_count: result?.adiadaCount ?? Number(review.adiada_count || 0) + 1 }
      setRevisoes((current) => current.map((item) => item.id === review.id ? updatedReview : item))
      setSelectedReview((current) => current?.id === review.id ? updatedReview : current)
      toast.success('Revisao movida para o proximo dia.')
    } catch (error) { toast.error(getUserMessage(error, 'Nao foi possivel mover a revisao.')) }
  }
  const finishReview = async (review) => {
    try {
      ...
      toast.success('RevisÃ£o concluÃ­da. PendÃªncias de reposiÃ§Ã£o foram mantidas.')
  ...
  const movePendencias = async () => {
    ...
    if (!selected.length) return toast.error('Selecione os itens para reposiÃ§Ã£o.')
    try {
      ...
      toast.success('Itens adicionados Ã  solicitaÃ§Ã£o.')
    } catch (error) { toast.error(getUserMessage(error, 'NÃ£o foi possÃ­vel adicionar os itens.')) }
  }
  ...
  return <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 pb-8">
    <header className="flex flex-col gap-4 rounded-2xl border bg-card p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-6">
      <div><h1 className="flex items-center gap-3 text-2xl font-bold"><ClipboardCheck className="text-primary" /> RevisÃµes</h1><p className="mt-1 text-sm text-muted-foreground">Rotinas de conferÃªncia, pendÃªncias e reposiÃ§Ã£o sem alterar o estoque.</p></div>
      <div className="flex w-full flex-wrap gap-2 sm:w-auto sm:justify-end"><PushNotificationToggle /><Button variant="outline" onClick={() => window.location.assign('/calendario')}><CalendarDays className="h-4 w-4" /> CalendÃ¡rio</Button><Button variant="outline" onClick={reload}><Clock3 className="h-4 w-4" /> Atualizar</Button></div></header>

    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      {[['Pendentes', summary.pendentes, 'border-t-amber-500'], ['Em andamento', summary.andamento, 'border-t-primary'], ['Atrasadas', summary.atrasadas, 'border-t-destructive'], ['Em falta', summary.emFalta, 'border-t-destructive'], ['Aguardando reposiÃ§Ã£o', summary.reposicao, 'border-t-emerald-500']].map(([label, value, tone]) => <Card key={label} className={'border-t-4 ' + tone}><CardContent className="p-4"><p className="text-sm font-medium text-muted-foreground">{label}</p><p className="mt-1 text-3xl font-bold tabular-nums">{value}</p></CardContent></Card>)}
    </div>

    <div className="flex flex-col gap-3 border-y py-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-semibold">Rotinas e revisÃµes</h2><p className="text-sm text-muted-foreground">Crie rotinas e execute as conferÃªncias que jÃ¡ estÃ£o programadas.</p></div><Dialog open={routineDialogOpen} onOpenChange={setRoutineDialogOpen}><DialogTrigger asChild><Button className="w-full sm:w-auto" onClick={() => { resetForm(); setRoutineDialogOpen(true) }}><Plus className="h-4 w-4" /> Criar rotina</Button></DialogTrigger><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>{editingId ? 'Editar rotina' : 'Nova rotina de revisÃ£o'}</DialogTitle></DialogHeader><Card className="border-0 shadow-none"><CardContent className="px-0">
      <form onSubmit={submitRoutine} className="grid gap-4 md:grid-cols-2">
        ...
        {form.tipo === 'produtos' && <div className="space-y-2 md:col-span-2"><div className="flex items-center justify-between gap-3"><Label>Categorias</Label><span className="text-xs text-muted-foreground">{form.categorias.length} selecionada{form.categorias.length === 1 ? '' : 's'}</span></div><div className="flex flex-wrap gap-2 rounded-lg border bg-muted/20 p-3">{categories.length === 0 ? <p className="text-sm text-muted-foreground">Nenhuma categoria cadastrada.</p> : categories.map((category) => <Button key={category} type="button" size="sm" variant={form.categorias.includes(category) ? 'default' : 'outline'} onClick={() => toggleCategory(category)}>{category}</Button>)}</div><p className="text-xs text-muted-foreground">Selecione uma ou mais categorias para incluir todos os produtos correspondentes na revisÃ£o.</p></div>}
        <div className="space-y-2"><Label>HorÃ¡rio</Label><Input type="time" value={form.horario} onChange={(e) => updateForm('horario', e.target.value)} /></div>
        <div className="space-y-2"><Label>FrequÃªncia</Label><Select value={form.frequencia} onValueChange={(value) => updateForm('frequencia', value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="diaria">DiÃ¡ria</SelectItem><SelectItem value="intervalo_dias">A cada X dias</SelectItem><SelectItem value="semanal">Semanal</SelectItem></SelectContent></Select></div>
        ...
        <div className="space-y-2"><Label>Foco</Label><Select value={form.foco} onValueChange={(value) => updateForm('foco', value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="inteligente">Inteligente (recomendado)</SelectItem><SelectItem value="estoque_baixo">Estoque baixo</SelectItem><SelectItem value="pendencias">PendÃªncias</SelectItem><SelectItem value="completa">Completa</SelectItem></SelectContent></Select></div>
        <div className="space-y-2"><Label>Repetir notificaÃ§Ã£o (minutos)</Label><Input type="number" min="5" value={form.repetir_notificacao_minutos} onChange={(e) => updateForm('repetir_notificacao_minutos', e.target.value)} /></div>
        <div className="space-y-2"><Label>Janela de revisÃ£o (minutos)</Label><Input type="number" min="30" value={form.janela_revisao} onChange={(e) => updateForm('janela_revisao', e.target.value)} /></div>
    ...
    <Card className="overflow-hidden">
      ...
      <CardContent className="space-y-4 p-4 sm:p-6">
        {pendencias.length === 0 ? <p className="text-sm text-muted-foreground">Nenhum item aguardando inclusao em solicitacao.</p> : <>
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)]">
            <div className="space-y-3">
              {pendenciaGroups.map((group) => {
                const selectedInGroup = group.items.filter((item) => selectedPendencias.includes(item.id)).length
                return <div key={group.key} className="overflow-hidden rounded-lg border bg-background">
                  <div className="flex items-center justify-between gap-3 border-b bg-muted/20 px-3 py-2">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-semibold" title={group.category}>{group.category}</p>
                      <p className="text-xs text-muted-foreground">{selectedInGroup} de {group.items.length} item(ns)</p>
                    </div>
                    <Button type="button" size="sm" variant="outline" onClick={() => setSelectedPendencias((current) => {
                      const ids = group.items.map((item) => item.id)
                      const allSelected = ids.every((id) => current.includes(id))
                      return allSelected ? current.filter((id) => !ids.includes(id)) : Array.from(new Set([...current, ...ids]))
                    })}>{selectedInGroup === group.items.length ? 'Limpar' : 'Selecionar'}</Button>
                  </div>
                  <div className="divide-y">
                    {group.items.map((item) => {
                      const produto = getPendenciaProduto(item)
                      const quantidade = getPendenciaQuantidade(item)
                      return <label key={item.id} className="grid cursor-pointer grid-cols-[auto_minmax(0,1fr)] gap-3 p-3 text-sm hover:bg-muted/40 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center">
                        <Checkbox checked={selectedPendencias.includes(item.id)} onCheckedChange={() => togglePending(item.id)} className="mt-1" />
                        <span className="min-w-0">
                          <span className="block break-words font-medium sm:truncate" title={item.nome_snapshot}>{produto.cod ? produto.cod + ' - ' + item.nome_snapshot : item.nome_snapshot}</span>
                          <span className="block text-xs text-muted-foreground sm:text-sm">{item.revisoes_estoque?.rotinas_revisao?.nome || 'Revisao'} · Estoque: {produto.estoque || 0} | Min: {produto.min || 0} | Max: {produto.max || 0}</span>
                        </span>
                        <span className="col-start-2 flex flex-wrap gap-2 sm:col-start-auto sm:flex-col sm:items-end">
                          <Badge variant={item.status === 'em_falta' ? 'destructive' : 'outline'}>{item.status === 'em_falta' ? 'EM FALTA' : 'REPOR'}</Badge>
                          <span className="rounded-md bg-muted px-2 py-1 text-xs font-semibold">Qtd: {quantidade}</span>
                        </span>
                      </label>
                    })}
                  </div>
              ...
              <TablePagination page={safePendenciasPage} totalPages={pendenciasTotalPages} totalItems={pendencias.length} pageSize={REVIEWS_PAGE_SIZE} itemLabel="pendencias" onPageChange={setPendenciasPage} />
            </div>
            ...
            <div className="space-y-4">
              <div className="rounded-lg border bg-background p-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Button type="button" size="sm" variant="outline" onClick={copyPasteList} disabled={!pasteListText}><Clipboard className="h-4 w-4" /> Copiar</Button>
                </div>
                <Select value={pasteCategory} onValueChange={setPasteCategory}>
                  ...
                  <SelectContent>
                    ...
                    {reposicaoListGroups.map((group) => <SelectItem key={group.key} value={group.key}>{group.category}</SelectItem>)}
                  </SelectContent>
                </Select>
                <Textarea className="mt-3 min-h-48 resize-y font-mono text-xs" readOnly value={pasteListText} placeholder="Selecione uma categoria." />
              </div>
              ...
              <div className="rounded-lg border bg-muted/20 p-3">
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-1">
                  <div className="space-y-2">
                    <Label>Destino</Label>
                    <Select value={targetSolicitacao} onValueChange={setTargetSolicitacao}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="nova">Criar listas por categoria</SelectItem>
                        {solicitacoesCompra.map((request) => <SelectItem key={request.id} value={request.id}>{request.codigo || request.nome_item}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                  {targetSolicitacao === 'nova' && <>
                    <div className="space-y-2">
                      ...
                      <div className="rounded-lg border bg-background p-3 text-sm text-muted-foreground">{selectedReposicaoBlocks.length === 0 ? 'Selecione itens para ver as listas.' : selectedReposicaoBlocks.map((block) => <div key={block.category} className="flex justify-between gap-3 py-1"><span className="truncate" title={block.category}>{block.category}</span><span className="shrink-0">{block.items.length} item(ns)</span></div>)}</div>
                    </div>
                    <div className="space-y-2">
                      <Label>Solicitante</Label>
                      <Select value={newList.solicitante_id || undefined} onValueChange={(value) => updateNewListField('solicitante_id', value)}>
                        <SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger>
                        <SelectContent>{activeSolicitantes.map((person) => <SelectItem key={person.id} value={person.id}>{person.nome}</SelectItem>)}</SelectContent>
                      </Select>
                    </div>
                    <div className="space-y-2">
                      <Label>Centro de custo</Label>
                      <Select value={newList.centro_custo_id || undefined} onValueChange={(value) => updateNewListField('centro_custo_id', value)}>
                        <SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger>
                        <SelectContent>{activeCentrosCusto.map((center) => <SelectItem key={center.id} value={center.id}>{getCentroCustoLabel(center)}</SelectItem>)}</SelectContent>
                      </Select>
                    </div>
                  </>}
                  <Button className="w-full" onClick={movePendencias} disabled={selectedPendencias.length === 0}><ShoppingCart className="h-4 w-4" /> {targetSolicitacao === 'nova' ? 'Solicitar compra' : 'Adicionar a lista'}</Button>
                </div>
              </div>
            </div>
          </div>
