...
export async function createPublicSolicitacao(form) {
  ...
  if (!payload.descricao) {
    throw new Error('Descricao do item e obrigatoria.')
  }
  ...
  if (!rpcError) {
    ...
    if (!result?.codigo) {
      throw new Error('Nao foi possivel gerar o codigo da solicitacao.')
    }
...
export async function getPublicSolicitacaoStatus(codigo) {
  ...
  if (!normalizedCode) {
    throw new Error('Informe o codigo da solicitacao.')
  }
...
export async function updateSolicitacaoCompra(id, updates) {
  ...
  const normalizedUpdates = {
    ...
    pedido: updates.pedido?.trim?.() ?? updates.pedido,
    nota_fiscal: updates.nota_fiscal?.trim?.() ?? updates.nota_fiscal
  }
