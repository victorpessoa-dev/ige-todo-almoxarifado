/**
 * Gerador HTML do catalogo de almoxarifado.
 *
 * O mesmo HTML atende catalogo interno e publico, por isso a montagem fica
 * centralizada para manter capa, sumario, ordenacao e impressao consistentes.
 */
/**
 * Escapa dados vindos do cadastro antes de interpolar no HTML.
 */
...
/**
 * Normaliza texto para busca, agrupamento e comparacao sem depender de acentos.
 */
...
/**
 * Resolve categoria padrao para produtos sem cadastro completo.
 */
...
/**
 * Divide marcas digitadas em formatos comuns de lista.
 */
...
/**
 * Remove marcas duplicadas preservando a escrita original exibida ao usuario.
 */
...
/**
 * Retorna a primeira marca para ordenacao secundaria do catalogo.
 */
...
/**
 * Divide produtos em paginas fixas para manter o catalogo imprimivel previsivel.
 */
...
/**
 * Cria ids HTML estaveis para secoes de categoria.
 */
...
/**
 * Conta categorias reais, ignorando o grupo padrao "Sem categoria".
 */
...
/**
 * Conta marcas unicas considerando listas separadas por virgula, ponto e virgula ou barra.
 */
...
/**
 * Filtra e ordena produtos do catalogo.
 *
 * A ordenacao prioriza categoria, marca e nome para deixar a consulta visual
 * consistente entre catalogo publico, interno e exportacao HTML.
 */
export function filterAndSortCatalogProducts(
  ...
  const selectedCategories = Array.isArray(selectedCategory)
    ? selectedCategory.map(normalizeCatalogText)
    : selectedCategory === 'todas'
      ? []
      : [normalizeCatalogText(selectedCategory)]
  const selectedBrands = Array.isArray(selectedBrand)
    ? selectedBrand.map(normalizeCatalogText)
    : selectedBrand === 'todas'
      ? []
      : [normalizeCatalogText(selectedBrand)]
  ...
  return produtos
    .filter((produto) => {
      ...
      const categoryMatch =
        ...
        selectedCategories.includes(normalizeCatalogText(category))
      const brandMatch =
        selectedBrands.length === 0 ||
        getUniqueBrandNames(produto.marcas).some(
          (brand) => selectedBrands.includes(normalizeCatalogText(brand))
        )
...
/**
 * Monta o HTML completo do catalogo.
 *
 * O sumario exibe paginas como referencia textual, sem links, para evitar
 * navegacao indesejada em visualizacao ou impressao.
 */
export function makeCatalogFrameHtml(produtos = []) {
  ...
  // Calcula paginas por categoria depois da paginacao, pois uma categoria pode
  // se estender por mais de uma pagina no arquivo final.
  ...
  const summaryEntries = [
    { label: 'Como ler o catalogo', detail: 'Explicacao dos campos', page: '3' },
  ...
  <main>
    <section class="cover" id="capa">
      <div class="cover-brand">
        <img src="/ige-supergesso.svg" alt="IGE Supergesso" />
