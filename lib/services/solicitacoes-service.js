...
/** Invalida as leituras publicas derivadas das tabelas de solicitacoes. */
export function invalidatePublicSolicitacaoCaches() {
  deleteMemoryCache(PUBLIC_SOLICITACOES_CACHE_KEY)
  deleteMemoryCache(PUBLIC_SOLICITANTES_CACHE_KEY)
  deleteMemoryCache(PUBLIC_CENTROS_CUSTO_CACHE_KEY)
}
...
export async function listPublicSolicitacoesStatus() {
  ...
  const detalhes = await Promise.all(
    solicitacoes.map(async (item) => {
      if (item.solicitante && item.centro_custo && 'previsao_desejada' in item) return item
      try {
...
export async function updateSolicitanteCompra(id, updates) {
  const { data, error } = await supabase
    ...
    .update({
      ...
      centro_custo_id: updates.centro_custo_id === undefined ? undefined : updates.centro_custo_id || null
    })
    .eq('id', id)
    .select()
    .single()
